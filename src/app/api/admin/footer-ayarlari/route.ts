...
// In-memory cache
let footerCache: { data: any; timestamp: number } | null = null;
...
export async function GET() {
  try {
    // Check cache first
    if (footerCache && (Date.now() - footerCache.timestamp) < CACHE_TTL) {
      const response = NextResponse.json({ success: true, data: footerCache.data });
    ...
    try {
      ...
      // Update cache
      footerCache = { data: settings, timestamp: Date.now() };
